...
grid = MISTBCGridv1("JWST") # hide
Teff = logrange(exp10(3.5), 10_000; length=1000) # hide
# logg = range(extrema(MISTBCGridv1).logg...; length=1000) # hide
...
The MIST v1.2 BC grid assumes scaled-solar metal abundance ratios using the protostellar solar abundances of [Asplund2009](@citet), so \[M/H\] is equivalent to \[Fe/H\]. We provide [`BolometricCorrections.MIST.MISTChemistryv1`](@ref) to access information on the v1.2 chemical mixture following the [chemical mixture API](@ref chemistry_api). `MISTChemistry` is provided as a deprecated alias for people transitioning from earlier versions of BolometricCorrections.jl prior to the addition of MIST v2.5.
...
```@docs
BolometricCorrections.MIST.MISTBCGridv1
```

The constructor for [`MISTBCGridv1`](@ref) includes a parser to translate most human-readable photometric system names (e.g., `"HST/ACS-WFC"`) into their proper internal identifiers. The full list of internal specifiers is given below.
...
```@docs
BolometricCorrections.MIST.MISTBCTablev1
```

### Deprecated aliases

`MISTBCGrid` and `MISTBCTable` are deprecated aliases for [`MISTBCGridv1`](@ref) and [`MISTBCTablev1`](@ref), respectively, provided for backwards compatibility. They forward all arguments to the v1 constructors and emit a deprecation warning if Julia is configured to show them (e.g., Julia started as `julia --depwarn=yes`).
